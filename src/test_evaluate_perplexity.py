...
lange samengestelde woorden en karakteristieke uitdrukkingen."""
    ...
    # Basic sanity checks
    assert not torch.isnan(torch.tensor(word_ppl))
    ...
    assert word_ppl < 1000  # Reasonable upper bound for coherent Dutch text
    assert token_ppl < 1000
    assert 1.0 < tpw < 5.0  # Typical range for Dutch subword tokenization
