...
def create_perplexity_distribution_plot(results_dict, output_dir):
    ...
    for model_name, results in results_dict.items():
        for dataset_name, data in results.items():
            if dataset_name in ['culturax_nl', 'mc4_nl', 'fineweb_edu_4']:
                ...
                # Access token_perplexities and word_perplexities from text_level_metrics
                token_ppls = [
                    text_data['token_perplexity']
                    for text_data in data['text_level_metrics']
                ]
                tokens_per_word = [
                    text_data['tokens_per_word']
                    for text_data in data['text_level_metrics']
                ]
                token_ppls_times_tpw = [
                    token_ppl * tpw
                    for token_ppl, tpw in zip(token_ppls, tokens_per_word)
                ]
                ...
                token_ppls_times_tpw = remove_outliers(token_ppls_times_tpw)
    ...
    for idx, dataset in enumerate(dataset_order):
        ...
        ppl_data = dataset_data[dataset_data['metric'].isin(['Token PPL', 'PPL * TPW'])]
        sns.violinplot(data=ppl_data, x='model', y='value', hue='metric',
                      ax=ax, palette=colors[:2], split=True, density_norm='count')
        ax.set_yscale('log')
    ...
    plt.suptitle('Perplexity and PPL * TPW Distribution Across Models',
                y=1.02, fontsize=16)
    plt.tight_layout()
...
def create_performance_comparison(results_dict, output_dir):
    ...
    if not results_dict or not any('performance' in dataset_results 
                                 for model_results in results_dict.values() 
                                 for dataset_results in model_results.values()):
        print("No performance metrics found in results. Skipping performance comparison plot.")
        return
