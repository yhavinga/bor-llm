...
def get_latest_checkpoint(remote_checkpoint_dir, local_checkpoint_dir, hostname):
    ...
    try:
        result = subprocess.run(done_check_cmd, shell=True, capture_output=True, text=True)
...
def run_evaluation(checkpoint_path, args):
    ...
    eval_dataset = load_dataset(args.dataset_name_openhermes, split="train")\
    ...
    while True:
        ...
        if checkpoint_path and checkpoint_path != last_evaluated_checkpoint:
            ...
            if args.cleanup_old_checkpoints:
                for old_checkpoint in glob.glob(os.path.join(args.local_checkpoint_dir, "checkpoint-*")):
