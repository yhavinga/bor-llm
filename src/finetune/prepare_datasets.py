...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prepare and save datasets for fine-tuning"
    )
    parser.add_argument(
...
def main():
    ...
    output_dir = os.path.join(
        get_project_root(), args.output_dir, f"{dataset_name}_{timestamp}"
    )
    ...
    if args.use_leesplank_dataset:
        ...
        train_dataset_leesplank = dataset_leesplank.select(
            range(
                args.eval_dataset_size, args.train_dataset_size + args.eval_dataset_size
            )
        )
        ...
        train_dataset = concatenate_datasets(
            [train_dataset_openhermes, train_dataset_leesplank]
        )
    else:
        eval_dataset = eval_dataset_openhermes
        train_dataset = train_dataset_openhermes

    dataset_dict = DatasetDict({"train": train_dataset, "validation": eval_dataset})
